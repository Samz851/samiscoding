...
<?php endif; ?>
	...
	<div class="uk-margin uk-grid uk-grid-small <?= $grid ?>" data-uk-grid-margin="">
		<?php foreach ($project->images as $image) : ?>
			<div style="max-width:30%; display:inline-block; background:rgba(255, 255, 255, 0.85); padding:3px;">
	...
	<?php if (in_array($config['project']['show_navigation'], ['both', 'bottom']) && ($next || $previous)) : ?>
		<ul class="uk-pagination">
		<?php if ($previous) : ?>
			<li class="uk-pagination-previous"><a href="<?= $app->url('@portfolio/id', ['id' => $previous->id]) ?>" class="samz-button">
					<i class="uk-icon-arrow-left uk-margin-small-right"></i><?= $previous->title ?></a></li>
		<?php endif; ?>
			<?php if ($next) : ?>
			<li class="uk-pagination-next"><a href="<?= $app->url('@portfolio/id', ['id' => $next->id]) ?>" class="samz-button">
					<?= $next->title ?><i class="uk-icon-arrow-right uk-margin-small-left"></i></a></li>
		<?php endif; ?>
		</ul>
	<?php endif; ?>
